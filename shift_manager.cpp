...
char current_shift = '\0';
uint16_t shift_counter = 0;

uint32_t shift_ok_total = 0;
uint32_t shift_ng_total = 0;
uint8_t  shift_rec_day = 0;
uint8_t  shift_rec_month = 0;
uint16_t shift_rec_year = 0;
...
static bool     shift_rollover_pending = false;
static char     pending_shift = '\0';
static uint8_t  pending_day = 0;
static uint8_t  pending_month = 0;
static uint16_t pending_year = 0;
// --------------------------------------------------------------------------

char compute_shift(uint8_t hour, uint8_t minute)
{
    uint16_t minutes_of_day = (uint16_t)hour * 60 + minute;
    return (minutes_of_day >= g_shift_a_min && minutes_of_day < g_shift_b_min) ? 'A' : 'B';
}

static bool build_mmyy_token(uint8_t month, uint16_t year, char out[5])
{
    if (month < 1 || month > 12)
        return false;
...
static bool parse_mmyy_token(const char token[5], uint8_t &month, uint16_t &year)
{
    if (token == nullptr)
        return false;
...
static bool ensure_month_stats_file(uint8_t month, uint16_t year)
{
    if (!littlefs_mounted)
        return false;
...
static void apply_shift_rollover(char new_shift, uint8_t day, uint8_t month, uint16_t year)
{
    ...
    shift_ng_total = 0;
    shift_counter = 0;
    hmi.Write_UString((uint16_t)STICKER, String(0)); // reset display the moment the shift actually changes
